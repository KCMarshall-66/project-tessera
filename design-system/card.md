...
- **Dashboard sections** (health-check tiles, task list, charts, team, concentration, scan mosaic) all use `.stat-card` / `.panel`, so they follow the spec. The dark investigation task cards only remain on Monitoring & Alerts, inside its flow panel.
