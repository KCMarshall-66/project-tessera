...
- **Dark surfaces are out of scope.** The white-fade spec doesn't apply to the dark data-flow hero on Data Mapping, which keeps its own treatment.
- **Not yet applied outside the app.** The prototype index cards, persona pages, and research page keep their earlier card style.

## Open work

- Decide whether the Data Mapping hero should adopt the open-top shape (bottom-only radius) so it matches the cards around it.
