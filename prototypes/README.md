...
- **AI sparkle (2026-09-21):** each health-check tile has the AI sparkle (`../brand/ai-sparkle.svg`) in its bottom-right corner, meant to open an advanced chat modal. The button is in place and dispatches a `tessera:ai-chat` event carrying the tile (`risk-score`, `open-tasks`, `arriving-vs-closed`, `team-score`); the modal itself is not built yet, so pressing it does nothing visible.
